...
void order_queue_remove(int floor,int direction){
	struct ORDER_QUEUE* current;
	struct ORDER_QUEUE* previous = root;
	current = previous->next;
	if(current == NULL)
		return;
	while (current->next != NULL){
		struct ORDER_QUEUE* temp = current;
		current = current->next;
		if(temp->floor == floor && temp->direction == direction){
			free(temp);
	...
	if (current->floor == floor && current->direction == direction){
		previous->next  = NULL;
		free(current);
	}
}
...
struct ORDER_QUEUE order_queue_peek(){
  struct ORDER_QUEUE temp;
  	if(root->next!=NULL){
  		struct ORDER_QUEUE* peeked = root->next;
  
  		temp.direction = peeked->direction;
  		temp.floor = peeked->floor;
  		temp.next=NULL;
